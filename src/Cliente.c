...
int main(int argc, char *argv[])
{
  ...
  snprintf(cliente.fifo_cliente, sizeof(cliente.fifo_cliente), FIFO_CLIENTE,
           cliente.pid_cliente);
  ...
  fd_servidor = abrirFIFO(
      FIFO_SERVIDOR,
      true); // abrir o FIFO do servidor para escrever o cliente de conexao

  write(fd_servidor, &cliente, sizeof(Cliente));
  close(fd_servidor);

  fd_cliente = abrirFIFO(
      cliente.fifo_cliente,
      false); // abrir o FIFO do cliente para ler a resposta do controlador
  ...
  if (strcmp(mensagem, REGISTADO) == 0)
  {
    ...
    while (escolheServico(cliente))
    { /* continua enquanto a função retornar 1*/
    }
  }
